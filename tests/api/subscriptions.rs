...
async fn subscribe_sends_confirmation_email_with_a_link() {
    ...
    let eamil_request = &app.email_server.received_requests().await.unwrap()[0];
    let confirmation_links = app.get_confirmation_links(eamil_request);
...
async fn subscribe_fails_if_there_is_a_fatal_database_error() {
    let app = spawn_app().await;
    let body = "name=le%20guin&email=ursula_le_guin%40gmail.com";
    let sql = "ALTER TABLE subscription_tokens DROP COLUMN subscription_token;";
    sqlx::query(sql).execute(&app.db_pool).await.unwrap();
